...
namespace MVC_Cultuurhuis.Services
{
    public class CultuurService
    {
        ...
        public List<Voorstelling> GetAlleVoorstellingenVanGenre(int? id)
        {
            ...
            {
                var query = from voorstelling in db.Voorstellingen.Include("Genre") where voorstelling.Datum >= DateTime.Today && voorstelling.GenreNr == id orderby voorstelling.Datum select voorstelling; return query.ToList(); } }
